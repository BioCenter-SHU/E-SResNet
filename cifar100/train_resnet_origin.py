...
def load_data(traindir, valdir, cache_dataset, distributed):
    ...
    if cache_dataset and os.path.exists(cache_path):
        ...
        if cache_dataset:
            ...
            utils.save_checkpoint((dataset, traindir), cache_path)
    ...
    if cache_dataset and os.path.exists(cache_path):
        ...
        if cache_dataset:
            ...
            utils.mkdir(os.path.dirname(cache_path))
            utils.save_checkpoint((dataset_test, valdir), cache_path)
...
def main(args):
    ...
    if args.model == 'E_SResNet_S':
        model = E_SResNet_S(in_chans=args.in_chans, T=args.T, remove_model_jit=args.remove_model_jit, AA=args.AA, SE=args.SE)
    elif args.model == 'E_SResNet_M':
        model = E_SResNet_M(in_chans=args.in_chans, T=args.T, remove_model_jit=args.remove_model_jit, AA=args.AA, SE=args.SE)
    else:
...
def parse_args():
    ...
    parser.add_argument('--in-chans', default=3, type=int, help='in_chans')

    parser.add_argument('--model', default='E_SResNet_M', help='model')
    ...
    parser.add_argument('--cos_lr_T', default=200, type=int,
    ...
    parser.add_argument('--AA', action='store_true', help='Use AntiAliasDownsampleLayer')
    parser.add_argument('--SE', action='store_true', help='Use SELayer')

    args = parser.parse_args()
    args.remove_model_jit = True
    return args
