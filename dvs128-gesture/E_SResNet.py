    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        if stride == 1:
            ...
            if anti_alias_layer is None:
                ...
                self.sn = MultiStepLIFNode(tau=1.5, detach_reset=True, backend='cupy')
...
class ESResNet(nn.Module):
    ...
    def __init__(self, layers, num_classes=10, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, T=4, remove_model_jit=False, AA=True, SE=True):
        ...
        self.layer1 = self._make_layer(BasicBlock, 16, layers[0], use_se=SE,
                                       anti_alias_layer=anti_alias_layer)
        self.layer2 = self._make_layer(BasicBlock, 32, layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0], use_se=SE, anti_alias_layer=anti_alias_layer)
        self.layer3 = self._make_layer(BasicBlock, 64, layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1], use_se=False, anti_alias_layer=anti_alias_layer)
...
def E_SResNet_T(**kwargs):
    return E_SResNet([3, 3, 3], **kwargs)
