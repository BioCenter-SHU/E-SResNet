...
class DownsampleJIT(object):
    ...
    def __call__(self, input: torch.Tensor):
        ...
        filt = self.filt.to(input.device)
        return F.conv2d(input, filt, stride=2, padding=0, groups=input.shape[1])
...
class Downsample(nn.Module):
    ...
    def forward(self, input):
        return F.conv2d(input, self.filt, stride=self.stride, padding=0, groups=input.shape[1])