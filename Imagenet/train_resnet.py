...
def main(args):
    ...
    output_dir += '_distributed_cosinelr'
    ...
    if args.model == 'E_SResNet_S':
        model = E_SResNet_S(in_chans=args.in_chans, T=args.T, remove_model_jit=args.remove_model_jit)
    elif args.model == 'E_SResNet_M':
        model = E_SResNet_M(in_chans=args.in_chans, T=args.T, remove_model_jit=args.remove_model_jit)
    else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if output_dir:
            ...
            if save_flag and utils.is_main_process():
                utils.save_on_master(
                    checkpoint,
                    os.path.join(output_dir, f'checkpoint_{epoch}.pth'))

            if save_max and utils.is_main_process():
                utils.save_on_master(
                    checkpoint,
...
def parse_args():
    ...
    parser.add_argument('--T', default=6, type=int, help='simulation steps')
